...
const prueba = require('./modulos/modulo_prueba');

console.log('==========');
console.log(prueba.unAuto);
console.log('==========');
...
http
	.createServer(function (req, res) {
		...
		switch (req.url) {
			// Home
			case '/':
				let soloTitulos = movies.map(function (unaMovie) {
				...
				contenidoHome += `Total de películas en cartelera: ${movies.length} \n\n`;
			...
			// Más Votadas
			case '/mas-votadas':
				let pelisMasVotadas = movies.filter((unaMovie) => unaMovie.vote_average >= 7);
